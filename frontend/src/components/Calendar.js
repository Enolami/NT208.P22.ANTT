...
const Calendar = () => {
    ...
    const [showGlobalSyncDialog, setShowGlobalSyncDialog] = useState(false);
    ...
    const fetchAllTasksAndEvents = async () => {
        ...
        try {
            ...
            const [local, google, outlook, eventRes] = await Promise.all([
                ...
                outlookConnected ? outlookService.sync() : Promise.resolve({ events: [], tasks: [] }),
                eventsAPI.getEvents()
            ]);
            console.log('[LOG] fetchAllTasksAndEvents finished');
            console.log('[Calendar] Raw eventRes.data:', eventRes.data);
            ...
            // Outlook
            const outlookEvents = (outlook.events || []).map(event => ({
            ...
            const outlookTasks = (outlook.tasks || []).map(task => ({
            ...
            console.log('[Calendar] Mapped eventModelEvents:', eventModelEvents);
    ...
    const getItemsForDay = (day) => {
        ...
        return { 
            localTasks: tasks.filter(task => 
                !task.external_provider && dayjs(task.start_time).isSame(dayStart, 'day')
            ),
            googleTasks: tasks.filter(task => 
                task.external_provider === 'google_tasks' && dayjs(task.due_date).isSame(dayStart, 'day')
            ),
            outlookTasks: tasks.filter(task => 
                task.external_provider === 'outlook_tasks' && dayjs(task.due_date).isSame(dayStart, 'day')
            ),
            googleEvents: tasks.filter(task => 
                task.external_provider === 'google_event' && dayjs(task.start_time).isSame(dayStart, 'day')
            ),
            outlookEvents: tasks.filter(task => 
                task.external_provider === 'outlook_event' && dayjs(task.start_time).isSame(dayStart, 'day')
            ),
            eventModelEvents: tasks.filter(task => 
                task.external_provider === 'event_model' && dayjs(task.start_time).isSame(dayStart, 'day')
            )
        };
    };
    ...
    const pushToCalendar = async (item, provider) => {
        console.log('[LOG] pushToCalendar called for', item, provider);
        try {
        if (provider === 'google') {
            if (item.external_provider === 'google_event' || item.external_provider === 'google_tasks') {
                console.log('[pushToCalendar] Skipping, already a Google item:', item);
                return;
            }
            try {
                if (item.end_time) {
                        console.log('[pushToCalendar] Pushing event to Google Calendar:', item);
                        await api.post('/calendarsync/events/push/', { 
                            id: item.id,
                            provider: 'google'
                        });
                } else {
                        console.log('[pushToCalendar] Pushing task to Google Tasks:', item);
            ...
            if (item.external_provider === 'outlook_event' || item.external_provider === 'outlook_tasks') {
                console.log('[pushToCalendar] Skipping, already an Outlook item:', item);
                return;
            }
            try {
                if (item.end_time) {
                        console.log('[pushToCalendar] Pushing event to Outlook Calendar:', item);
                        await api.post('/calendarsync/outlook/push_events/', { 
                            id: item.id,
                            provider: 'outlook'
                        });
                } else {
                        console.log('[pushToCalendar] Pushing task to Outlook Tasks:', item);
    ...
    // Global Sync: Push all data to all connected calendars, avoiding duplicates
    const handleGlobalSync = async () => {
        console.log('[LOG] handleGlobalSync called');
        setGlobalSyncLoading(true);
        setGlobalSyncMessage('');
        try {
            console.log('[GlobalSync] Starting global sync. All items:', tasks);
            // Gather all items
            const allItems = [...tasks];
            // For each provider, push items that do not have external_id for that provider
            const promises = [];
            // Google
            if (tasks.some(t => t.external_provider === 'google_event' || t.external_provider === 'google_tasks')) {
                allItems.forEach(item => {
                    if (!item.external_id || (item.external_provider !== 'google_event' && item.external_provider !== 'google_tasks')) {
                        console.log('[GlobalSync] Will push to Google:', item);
            ...
            if (tasks.some(t => t.external_provider === 'outlook_event' || t.external_provider === 'outlook_tasks')) {
                allItems.forEach(item => {
                    if (!item.external_id || (item.external_provider !== 'outlook_event' && item.external_provider !== 'outlook_tasks')) {
                        console.log('[GlobalSync] Will push to Outlook:', item);
                        promises.push(pushToCalendar(item, 'outlook'));
                    }
                });
            }
            await Promise.all(promises);
            // Refresh data and trigger debug logs
            if (tasks.some(t => t.external_provider === 'outlook_event' || t.external_provider === 'outlook_tasks')) {
